...
export function useRecettes() {
  const [recettes, setRecettes] = useState(RECETTES_INITIALES);
    ...
    (async () => {
      try {
        ...
        if (raw) {
          ...
          if (!cancelled) {
            // Combiner les recettes en dur avec celles du localStorage
            const recettesStockees = parsed.filter(r => r.id > 1000); // Recettes créées par l'utilisateur
            const recettesCombinees = [...RECETTES_INITIALES, ...recettesStockees];
            setRecettes(recettesCombinees);
            console.log('✅ Recettes chargées:', recettesCombinees);
          }
        } else {
  ...
  useEffect(() => {
    ...
    try {
      // Ne sauvegarder que les recettes créées par l'utilisateur (id > 1000)
      const recettesAStocken = recettes.filter(r => r.id > 1000);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(recettesAStocken));
      console.log('💾 Recettes utilisateur sauvegardées:', recettesAStocken);
    } catch (e) {
